...
import { Code, Server, Database, TerminalSquare, Palette, Wrench } from 'lucide-react';
      ...
      >
        ...
        <div className="flex flex-wrap justify-center mb-12 gap-2">
          {categories.map((category) => (
            <button
              key={category}
              className={`px-4 py-2 rounded-full transition-all duration-300 flex items-center gap-2 ${activeCategory === category
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[
            ...
            { name: 'JavaScript', icon: '📜', color: 'bg-yellow-100 dark:bg-yellow-900/30' },

            { name: 'React', icon: '⚛️', color: 'bg-blue-100 dark:bg-blue-900/30' },
            { name: 'Node.js', icon: '🟢', color: 'bg-green-100 dark:bg-green-900/30' },
            { name: 'Redux Saga', icon: '🔁', color: 'bg-purple-100 dark:bg-purple-900/30' },

            { name: 'Tailwind CSS', icon: '🌊', color: 'bg-teal-100 dark:bg-teal-900/30' },

            { name: 'Postman', icon: '📬', color: 'bg-orange-100 dark:bg-orange-900/30' },


            { name: 'GitHub', icon: '🐙', color: 'bg-gray-100 dark:bg-gray-800/30' },
            { name: 'MySQL', icon: '🐬', color: 'bg-blue-100 dark:bg-blue-900/30' },
            { name: 'MongoDB', icon: '🍃', color: 'bg-green-100 dark:bg-green-900/30' }
          ].map((tool, index) => (
            <motion.div
              key={tool.name}
